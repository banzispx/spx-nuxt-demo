<template>
  ...
  >
    ...
    >
      <div class="tip_text">
        <!-- 横纵坐标名称 河流名称-->
        <p style="margin-top:-25px;font-weight:800;font-size:16px;">
          {{ getRiverName }}
        </p>
        <p class="tip_text_set tip_text_set_heng">
          距离(km)
        </p>
        <p class="tip_text_set tip_text_set_zong tip_text_set_zong_left">
          高程(m)
        </p>
        <p class="tip_text_set tip_text_set_zong tip_text_set_zong_right">
          高程(m)
        </p>
      </div>
      ...
      <div
        ref="showSvgContent"
        class="chart_content_element"
      >
        <svg
          :width="svgWidth"
          :height="svgHeight"
          class="svg_content_element"
        >
          <g
            class="layer"
          ...
          >
            ...
            <g v-if="isShowDamIdInfo">
              <g
                v-for="(item, index) in damIds"
                :key="'text'+index"
              >
                <!-- {{showDamIdInfo.dbid !== item.dbid ? item.dbmc[i-1] : ''}} -->
                <text
                  v-for="(i) in item.dbmc.length+1"
                  :key="'dam'+i"
                  class="dam_name_text text_middle"
                  :x="chartWidth * (item.lat-Xmin) / (Xmax-Xmin)"
                  :y="chartHeight - (chartHeight) * item.lon / Ymax + 10 + (i-1)*20+20"
                >{{ showDamIdInfo.dbid !== item.dbid ? item.dbmc[i-1] : '' }}</text>
              </g>
            </g>
            <g v-else>
              <g
            ...
            <g
              v-for="i in y0damIds.length-1"
              :key="'red'+i"
            >
              <g v-if="y0damIds[i].pre.level">
                <path
                  name="三角形"
                  :fill="`${y0damIds[i].damState === '已注册' || y0damIds[i].damState === '已建' ? 'url(#MyGradient)' : trangleInfo.normal.bgColor }`"
                  stroke="transparent"
                  :d="`M${Number.isNaN(y0damIds[i].pre.lat1) ? 0 : y0damIds[i].pre.lat1} ${y0damIds[i].pre.lon1} L${Number.isNaN(y0damIds[i].pre.lat) ? 0 : y0damIds[i].pre.lat} ${y0damIds[i].pre.lon} L${y0damIds[i].lat} ${y0damIds[i].pre.lon} L${y0damIds[i].lat} ${y0damIds[i].lon}  Z`"
                ...
                <defs>
                  ...
                  >
                    <stop
                      offset="0%"
                      :stop-color="trangleInfo.active.bgColor"
                    />
                    <stop
                      offset="100%"
                      stop-color="rgb(229,248,249)"
                    />
                  </linearGradient>
                </defs>
                <!-- 三角形 -->
                <line
                  :x1="Number.isNaN(y0damIds[i].pre.lat) ? 0 : y0damIds[i].pre.lat"
                  :y1="y0damIds[i].pre.lon"
                  :x2="y0damIds[i].lat"
                  :y2="y0damIds[i].pre.lon"
                  :style="`stroke:${y0damIds[i].damState === '已注册' || y0damIds[i].damState === '已建' ? trangleInfo.active.rowColor : trangleInfo.normal.rowColor };stroke-width:1;cursor:pointer;`"
                  :stroke-dasharray="`${y0damIds[i].damState === '已注册' || y0damIds[i].damState === '已建' ? '' : '4 2'}`"
                />
                <!-- 横线 -->
                <g
                  v-if="Math.abs(y0damIds[i].pre.lat-y0damIds[i].lat)>58"
                  transform="translate(-16, 0)"
                >
                  <image
                    xlink:href="@/static/waterjson/sjx.png"
                    preserveAspectRatio="none"
                    :x="(y0damIds[i].pre.lat+y0damIds[i].lat)/2"
                    :y="y0damIds[i].pre.lon-14"
                    width="11"
                    height="11"
                  />
                  <text
                    class="scale_text text_middle"
                    :x="(y0damIds[i].pre.lat+y0damIds[i].lat)/2 + 26"
                    :y="y0damIds[i].pre.lon-4"
                  >
                ...
                <line
                  v-if="y0damIds[i].damState === '已注册' || y0damIds[i].damState === '已建'"
                  :x1="y0damIds[i].lat"
                  :y1="y0damIds[i].lon-y0damIds[i].height*chartHeight / Ymax"
                  :x2="y0damIds[i].lat"
                  :y2="y0damIds[i].lon"
                  :style="`stroke:${y0damIds[i].domination === '水利' ? trangleInfo.active.waterColColor : trangleInfo.active.colColor};stroke-width:${damItemWidth};`"
                />

                <g v-else>
                  <line
                    :x1="y0damIds[i].lat-damItemWidth/2"
                    :y1="y0damIds[i].lon-y0damIds[i].height*chartHeight / Ymax"
                    :x2="y0damIds[i].lat-damItemWidth/2"
                    :y2="y0damIds[i].lon"
                    :style="`stroke:${trangleInfo.normal.colColor};stroke-width:2;`"
                    stroke-dasharray="3 2"
                  />
                  <line
                    :x1="y0damIds[i].lat+damItemWidth/2-1"
                    :y1="y0damIds[i].lon-y0damIds[i].height*chartHeight / Ymax"
                    :x2="y0damIds[i].lat+damItemWidth/2-1"
                    :y2="y0damIds[i].lon"
                    :style="`stroke:${trangleInfo.normal.colColor};stroke-width:2;`"
                    stroke-dasharray="3 2"
                  />
            ...
            <g v-if="isShowDamIdInfo">
              <!-- 横坐标需要修改偏移量 -->
              <line
                :x1="showDamIdInfo.lat"
                y1="-20"
                :x2="showDamIdInfo.lat"
                :y2="svgHeight"
                :style="`stroke:${trangleInfo.normal.rowColor};stroke-width:1;cursor:pointer;`"
                stroke-dasharray="5 3"
              />
              <text
                v-for="(i) in showDamIdInfo.dbmc.length+1"
                :key="'dam'+i"
                class="mark_text"
                :x="showDamIdInfo.lat"
                :y="showDamIdInfo.lon+ (i-1)*26+26"
              >{{ showDamIdInfo.dbmc[i-1] }}</text>
              <image
                xlink:href="@/static/waterjson/red.png"
                preserveAspectRatio="none"
                :x="showDamIdInfo.lat-22"
                :y="showDamIdInfo.lon-48"
                width="44"
                height="44"
                style="opacity:0.9;"
                @mouseover="showInfo(showDamIdInfoNum)"
                @mouseout="isShowInfo=false"
              />
            ...
            <g class="x axis">
              <line
                ...
                :x2="svgWidth"
                :y2="chartHeight+100"
                style="stroke:rgb(202, 215, 228);stroke-width:2"
              />
              <!-- 刻度 -->
              <g v-if="isShowDamIdInfo">
                <text
                  v-for="item in 11"
                  :key="'xt' + item"
                  class="scale_text text_start"
                  :x="(item-1)*Xmax/10*chartWidth/Xmax"
                  :y="12 + chartHeight + scaleBetweenPre + 100"
                >{{ Number(Number(Xmin+(item-1)*(Xmax-Xmin)/10) - showDamIdInfo.damDistance).toFixed(1) }}</text>
              </g>
              <g v-else>
                <text
                  v-for="item in 11"
                  ...
                  :x="(item-1)*Xmax/10*chartWidth/Xmax"
                  :y="12 + chartHeight + scaleBetweenPre + 100"
                >{{ Number(Number(Xmin+(item-1)*(Xmax-Xmin)/10)).toFixed(1) }}</text>
              </g>
              <line
                v-for="item in 11"
                :key="'xl' + item"
                :x1="(item-1)*Xmax/10*chartWidth/Xmax"
                :y1="chartHeight+ 100"
                :x2="(item-1)*Xmax/10*chartWidth/Xmax"
                :y2="chartHeight + scaleBetweenPre/2 + 100"
                :style="`stroke:${scaleColorSmall};stroke-width:${scaleColorSmallWidth}`"
              />
              <!-- 坐标轴名称 -->
      ...
      <svg
        width="80"
        :height="svgHeight"
        class="svg_position svg_fixed_y"
      >
        <g
          class="layer"
          transform="translate(80,20)"
        >
          <g class="y axis">
            ...
            <text
              ...
              :x="-scaleBetweenPre"
              :y="chartHeight+100"
            >{{ -Number(100*Ymax/chartHeight).toFixed(0) }}</text>
            <line
              :x1="-scaleBetweenPre/2"
              :y1="chartHeight+100"
              x2="0"
              :y2="chartHeight+100"
              :style="`stroke:${scaleColorSmall};stroke-width:${scaleColorSmallWidth}`"
            ...
            <text
              ...
              style="text-anchor:end;"
              :x="-scaleBetweenPre"
              :y="4 + (item-1)*(Ymax/3)*(chartHeight)/Ymax"
            >{{ Number(Ymax - Ymax/3*(item-1)).toFixed(0) }}</text>
            <line
              v-for="item in 4"
              :key="'l'+item"
              :x1="-scaleBetweenPre/2"
              :y1="(item-1)*(Ymax/3)*(chartHeight)/Ymax"
              x2="0"
              :y2="(item-1)*(Ymax/3)*(chartHeight)/Ymax"
              :style="`stroke:${scaleColorSmall};stroke-width:${scaleColorSmallWidth}`"
            />
            <!-- 纵坐标名称 -->
      ...
      <svg
        width="80"
        :height="svgHeight"
        class="svg_fixed_y svg_position_right"
      >
        <g class="layer">
          <g
            class="y axis"
    ...
    <div
      ...
      ref="damInfo"
      class="dam_info"
      :style="`left:${showInfoLeft};top:${showInfoTop}`"
    >
      ...
      <p v-if="isShowDamIdInfo">
        大坝距离： {{ Number(selectedInfo.lengthToHead/1000 - showDamIdInfo.damDistance).toFixed(1) }}km
      </p>
      <p v-else>
        大坝距离： {{ Number(selectedInfo.lengthToHead/1000).toFixed(1) }}km
      </p>
    </div>
  </div>
</template>
  ...
  methods: {
    ...
    initData() {
      ...
      if (this.Xmax < this.standardWidth) {
        ...
        this.chartWidth = this.standardWidth;
        this.svgWidth = this.standardWidth + 500;
      } else {
        this.chartWidth = this.Xmax;
        this.svgWidth = this.Xmax + 500;
      }
      // 最大值 y
    ...
    computePointPre(preLat, preLon, curLat, curLon, damHeight, normalLevel) {
      ...
      if (preLat !== 0 && valLat <= preLat) {
        ...
        lat1 = valLat;
        lon1 = preLon;
      } else if (preLat === 0 && valLat > 0 && valLat < curLat) {
        // valLat = preLat + 4;
        lat1 = valLat;
        lon1 = valLon;
      } else if (preLat === 0) {
